...
def merge_distance(T, I_tile, threshold = 1420.4): #1037.916
...
def merge(ref, alt, bi_align_field, tile_size = 32, stride = 16):
...
#                 print("i, j, i%32, j%32", i, j, i%32, j%32)
                I_tile = T.copy()
            else:
                I_tile = alt[xs:xe, ys:ye]
            weight = merge_distance(T, I_tile)
            merged_tile = weight * I_tile + (1-weight) * T
            merged_frame[i:i+tile_size, j:j+tile_size] += merged_tile.astype(np.float64) * raised_cosine_filter()
